...
def main(opt):
    ...
    net_pred = DAFCN(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
                     num_stage=opt.num_stage, dct_n=opt.dct_n)
    net_pred.cuda()

    assert opt.ckpt is not None, 'Please specify the checkpoint to test'
...
def run_model(net_pred, is_train=0, data_loader=None, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out_all = net_pred(p3d_src, input_n=in_n, output_n=(10 if dct_n == 20 else 25), itera=itera)

        p3d_out_all = p3d_out_all[:, seq_in:].transpose(1, 2).reshape(
